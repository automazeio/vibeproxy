...
final class ResponsesSSEFramerTests: XCTestCase {
    func testFramesPartialResponsesEventAcrossChunks() {
    ...
    func testRelayReframesResponsesSSEAfterHeaders() {
    ...
    func testRelayPassthroughForNonResponsesRequest() {
    ...
    func testFramerHandlesSlicedDataWithoutIndexTrap() {
        let framer = ResponsesSSEFramer()
    ...
    func testRelayDecodesChunkedEventStreamResponses() {
        let relay = HTTPResponseRelay(requestPath: "/v1/chat/completions")
        let raw = Data((
            "HTTP/1.1 200 OK\r\n" +
            "Content-Type: text/event-stream\r\n" +
